...
- **`Customer`** - Customer information with optional employee reference for discounts (uses PersonBase)
- **`Employee`** - Staff members with roles, wage tracking, and discount privileges (uses PersonBase)
...
- **Employee Discounts**: Staff members receive configurable percentage discounts through `employeeId` reference
...
- **Pricing Calculations**: Base video pricing with automatic employee discount application
...
- **Role Hierarchy**: Manager, Shift Supervisor, Clerk with configurable permissions
- **Dual Customer Role**: Employees can rent as customers with automatic discount application
- **Wage Tracking**: Hourly wage management with money value object
- **Active Status**: Employee lifecycle management with activation/deactivation
...
- **PersonBase Pattern**: Shared base model eliminates duplication between Customer and Employee
...
- **Business Logic**: Domain-specific constraints (rental periods, discount percentages, employee roles)
- **Status Enums**: Comprehensive status tracking for rentals, payments, inventory, customers, employees
...
- **Customer Management**: Complete customer lifecycle with employee integration
...
- **Employee Operations**: Staff management with role-based permissions and dual customer role
